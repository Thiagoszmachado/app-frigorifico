...
def fetch_abates() -> pd.DataFrame:
    ...
    df["mes_nome"] = pd.to_datetime(df["data_abate"], errors="coerce").dt.month_name(locale="pt_BR").str.upper()
    # Corrige nomes ingleses se necessário
    mapa = {
    ...
    df["mes_nome"] = df["mes_nome"].replace(mapa)
